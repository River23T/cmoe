...
"""Phase 2b play/evaluation script — Phase 2a + VAE 输出接到 actor.

Phase 2b actor 输入 134-dim = [policy(99), v_pred(3), z_H(32)].
...
关键差异 vs play_phase2.py:
  1. CALL inject_vae_actor (Phase 2b 必须有 VAE 才能跑)
  2. CALL inject_phase2b_runner_patches (load checkpoint 容错)
  3. task = "CMoE-Phase2b-VAE-Actor-Play-v0"
"""
...
parser = argparse.ArgumentParser(description="Phase 2b (VAE → actor) eval.")
parser.add_argument("--num_envs",   type=int,   default=64)
parser.add_argument("--task",       type=str,   default="CMoE-Phase2b-VAE-Actor-Play-v0")
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    # ============================================================
    # Phase 2b: 必须 inject VAE+actor (actor 输入 134 dim 必须靠 VAE 才能算)
    # ============================================================
    agent_dict = agent_cfg.to_dict()
    cmoe_cfg = agent_dict.get("algorithm", {}).pop("cmoe_cfg", None)
    if cmoe_cfg is None:
        raise RuntimeError("[Phase2b-Play] cmoe_cfg missing — task config error")
    print(f"[Phase2b-Play] popped cmoe_cfg with keys: {list(cmoe_cfg.keys())}, phase={cmoe_cfg.get('phase')}")

    runner = OnPolicyRunner(env, agent_dict, log_dir=log_dir, device=agent_cfg.device)

    # Phase 2b inject (creates VAE, augments actor 99→134, wraps act/update)
    print("[Phase2b-Play] injecting VAE + actor extension...")
    ...
    # 加载 checkpoint (patched runner.load 会处理 99→134 dim 转换)
    print(f"[Phase2b-Play] loading checkpoint: {resume_path}")
    ...
    for ep_idx in range(args_cli.n_episodes):
        ...
        for t in range(max_steps):
            ...
            if ep_idx == 0 and t == 0:
                ...
                print(f"[Phase2b-Play SANITY] step 0: action_norm_mean={a_norm:.4f}, "
                      f"action_abs_max={a_max:.4f}")
                if a_norm < 0.05:
                    print(f"[Phase2b-Play SANITY] ⚠️  action norm extremely low — "
                          f"policy may be outputting near-zero actions.")
                else:
                    print(f"[Phase2b-Play SANITY] ✓ actor producing nontrivial actions")
    ...
    print()
    print("═" * 70)
    print("PHASE 2b (VAE → actor) RESULTS — flat ground, std cmd 0.8 m/s")
    print("═" * 70)
    ...
    if np.mean(all_success) > 0.9 and np.mean(all_dist) > 7.0:
        print("  ✅ PHASE 2b PASSED. Walking with VAE-augmented actor ✓")
        print("     说明: actor 用 v_pred + z_H (而非 base_lin_vel ground truth) 也能 walk")
        print("     这是 paper Fig.3 的 systematic obs 思路 (部分实现)")
        print("     下一步: Phase 3 - 加 AE estimator (elevation map) + 第一种地形")
    elif np.mean(all_success) > 0.5:
        print("  ⚠️  Walking 部分退化, actor 在 134-dim 输入下没完全收敛")
        print("     可能原因: warm-start 后训练 iter 不够; VAE 还没收敛")
        print("     建议: 继续训练 (--resume 加 1500 iter) 或检查 vae_loss 是否下降")
    else:
        print("  ❌ PHASE 2b FAILED. Walking 严重退化")
        print("     可能原因:")
        print("       1. actor 第一层扩展 99→134 时旧权重没正确复制 (检查 [Phase2b] log)")
        print("       2. VAE 输出 v_pred 跟 base_lin_vel 差距太大, 误导 actor")
        print("       3. warm-start 没成功 (检查 [Phase2b-LOAD] log)")
    print("═" * 70)
