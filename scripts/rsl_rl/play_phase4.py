...
"""Phase 4 play/evaluation script — Phase 2a + VAE 输出接到 actor.

Phase 4 actor 输入 134-dim = [policy(99), v_pred(3), z_H(32)].
...
关键差异 vs play_phase2.py:
  1. CALL inject_moe_swav (Phase 4 必须有 VAE 才能跑)
  2. CALL inject_phase4_runner_patches (load checkpoint 容错)
  3. task = "CMoE-Phase4-MoE-CL-Play-v0"
"""
...
parser = argparse.ArgumentParser(description="Phase 4 (VAE → actor) eval.")
parser.add_argument("--num_envs",   type=int,   default=64)
parser.add_argument("--task",       type=str,   default="CMoE-Phase4-MoE-CL-Play-v0")
...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: RslRlBaseRunnerCfg):
    ...
    # ============================================================
    # Phase 4: 必须 inject VAE+actor (actor 输入 134 dim 必须靠 VAE 才能算)
    # ============================================================
    agent_dict = agent_cfg.to_dict()
    cmoe_cfg = agent_dict.get("algorithm", {}).pop("cmoe_cfg", None)
    if cmoe_cfg is None:
        raise RuntimeError("[Phase4-Play] cmoe_cfg missing — task config error")
    print(f"[Phase4-Play] popped cmoe_cfg with keys: {list(cmoe_cfg.keys())}, phase={cmoe_cfg.get('phase')}")

    runner = OnPolicyRunner(env, agent_dict, log_dir=log_dir, device=agent_cfg.device)

    # Phase 4 inject (creates VAE, augments actor 166 (5 experts), wraps act/update)
    print("[Phase4-Play] injecting VAE + actor extension...")
    ...
    print(f"[Phase4-Play] loading checkpoint: {resume_path}")
    ...
    for ep_idx in range(args_cli.n_episodes):
        ...
        for t in range(max_steps):
            ...
            if ep_idx == 0 and t == 0:
                ...
                print(f"[Phase4-Play SANITY] step 0: action_norm_mean={a_norm:.4f}, "
                      f"action_abs_max={a_max:.4f}")
                if a_norm < 0.05:
                    print(f"[Phase4-Play SANITY] ⚠️  action norm extremely low — "
                          f"policy may be outputting near-zero actions.")
                else:
                    print(f"[Phase4-Play SANITY] ✓ actor producing nontrivial actions")
    ...
    print()
    print("═" * 70)
    print("PHASE 4 (MoE+SwAV) RESULTS — flat ground, std cmd 0.8 m/s")
    print("═" * 70)
    ...
    if np.mean(all_success) > 0.9 and np.mean(all_dist) > 7.0:
        print("  ✅ PHASE 4 PASSED. Walking with MoE+SwAV ✓")
        print("     说明: actor 由 5 个 experts 组成, gating 网络接 z_E")
        print("     critic 也由 5 experts + 共享 gating, paper §III-D 严格")
        print("     SwAV contrastive: gate logits ↔ z_E (paper §III-E)")
        print("     注: 平地下 z_E 几乎常数, 5 experts 可能仍学到相似行为 — 这是预期的")
        print("     下一步: Phase 5 - 引入 8 种地形 + curriculum + 域随机化")
    elif np.mean(all_success) > 0.5:
        print("  ⚠️  Walking 部分退化, MoE 5 experts 在 166-dim 输入下没完全收敛")
        print("     可能原因: warm-start 后训练 iter 不够; gating 仍随机")
        print("     建议: 继续训练 (--resume 加 1500 iter) 或检查 swav_loss 是否下降")
    else:
        print("  ❌ PHASE 4 FAILED. Walking 严重退化")
        print("     可能原因:")
        print("       1. 5 experts warm-start 没成功 (检查 [Phase4-LOAD] copied count)")
        print("       2. gating 输出极端 → softmax 退化 (检查 swav_loss)")
        print("       3. SwAV 干扰 PPO 训练 (尝试 loss_weight=0.0)")
    print("═" * 70)
